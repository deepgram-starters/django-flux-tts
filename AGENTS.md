...
Python (Django Channels + Daphne) demo app for Deepgram Flux streaming text-to-speech (v2 Speak).

## Architecture

- **Backend:** Python (Django Channels + Daphne) on port 8081
...
| File | Purpose |
|------|---------|
| `app.py` (entry) + `starter/consumers.py` (WebSocket consumer) | Backend — HTTP endpoints + Deepgram `speak.v2` WebSocket bridge |
...
- `TtsConsumer` (an `AsyncWebsocketConsumer`) drives the async `AsyncDeepgramClient` `speak.v2`; served by Daphne.
